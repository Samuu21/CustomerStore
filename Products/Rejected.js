...
const Rejected = ({ navigation, route }) => {
    ...
    useEffect(() => {
        const fetchProducts = async() => {
            const q = query(collection(db, "Items"), where("supplier", "==", route.params.id), where("status", "==", 'Rejected'));
    ...
    {
        const q = query(collection(db, "Items"), where("supplier", "==", supplier), where("name", "==", name), where("top", "==", top),  where("bottom", "==", bottom), where("bra", "==", bra), where("cup", "==", cup), where("status", "==", 'Rejected'));
        const querySnapShot = await getDocs(q);
        if(querySnapShot.empty)
        {
            //
            ...
            <View style = {styles.container}>
                ...
                <ScrollView style = {styles.scrollV}>
                    <View style = {styles.scroll}>
                        ...
                        <Text style = {styles.desc}> There are no items rejected yet. We are on it!</Text>
                    </View>
                </ScrollView>
                <Modal
                ...
                <ImageBackground source = {require('../images/cover9.png')} resizeMethod = 'scale'
                    ...
                    {products.map((item, key) => (
                        <TouchableOpacity style = {styles.hold}
                            key = {key}
                            onPress = { () => {navigation.navigate('Item', {name: item.name, top: item.top, bottom: item.bottom, cup: item.cup, collection: 'Rejected'})}}>
                            <View style = {styles.row}>
                                <View style = {styles.first}>
                                    <TouchableOpacity style = {styles.delete}
                                        onPress = {() => {deleteData(item.supplier, item.name, item.top, item.bottom, item.cup)}}>
                                ...
                                <View style = {styles.info}>
                                    <ScrollView>
                                        ...
                                        <Text style = {styles.text}>Product size: Top Size - {item.top}, Bottom Size - {item.bottom}, Bra Size - {item.bra}{item.cup}</Text>
                                        <Text style = {styles.text}>Product price: {item.price}</Text>
                                        <Text style = {styles.text}>Product gender: {item.gender}</Text>
                                        <Text style = {styles.text}>Product category: {item.category}</Text>
                                        <Text style = {styles.text}>Product sale: {item.sale}</Text>
                                        <Text style = {styles.text}>Product saleprice: {item.saleprice}</Text>
                                    </ScrollView>
                                </View>
                            </View>
                            <View style = {styles.status}>
                                <Text style = {styles.label}>Product Rejection Reason:</Text>
