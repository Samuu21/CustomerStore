...
import Details_B from "./Details/Details_B";
...
const Bank = () => {
    ...
    const [bank, setBa] = useState('');
    const [branch, setBr] = useState('');
    ...
    useEffect(() => {
        const fetchid = async() => {
            ...
            setId(userid);
            const q = query(collection(db, "Bank"), where("supplier", "==", userid));
            ...
            else
            {
                querySnapShot.forEach((doc) => {
                    setAcc(doc.data().account);
                    setName(doc.data().name);
                    setBa(doc.data().bank);
                    setBr(doc.data().branch);
                    setDocId(doc.id);
                });
            }
        };
        ...
        <View style = {styles.container}>
            ...
            <ImageBackground source = {require('../images/cover4.png')} resizeMethod = 'scale'
                    ...
                    <View style = {styles.user}>
                        ...
                        <View style = {styles.row}>
                            <View style = {styles.hold}>
                                <Text style = {styles.header}>Cardholder Name:</Text>
                ...
                <Modal
                    ...
                    <View style = {styles.container}>
                        <Details_B id = {id} c = {name} br = {branch} ba = {bank} acc = {account} doci = {docId}/>
