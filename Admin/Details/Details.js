...
const Details = ({ ho, st, to, po, doci }) => {
    ...
    async function updateData(n, s, t, p, doci) 
    {
        try {
            await setDoc(doc(db, 'Destination', 'doci', doci),
            {
                number: n,
                street: s,
                town: t,
            ...
            alert('Your destination details have been updated.');
        } catch (error) {
            console.error("Error adding document: ", error);
        }    
    }

    async function addData(n, s, t, p) 
    {
        try {
            const docRef = await addDoc(collection(db, 'Destination'),
            {
                number: n,
                street: s,
                town: t,
            ...
            alert('Your destination details have been inserted.');
        } catch (error) {
            console.error("Error adding document: ", error);
        }    
    }
        ...
        return (
            <View style = {styles.container}>
                <ScrollView style = {{ flex: 1, minHeight: '100%' }}>
                    <Text style = {styles.header}>Destination Details Form</Text>
        ...
        return (
            <View style = {styles.container}>
                <ScrollView style = {{ flex: 1, minHeight: '100%' }}>
                    ...
                    <TouchableOpacity style = {styles.button}
                        onPress = {() => {updateData(n, s, t, p, doci)}}>
                        <Text style = {styles.buttonLabel}>Save</Text>
                    </TouchableOpacity>
                </ScrollView>
            </View>
