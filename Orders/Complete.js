...
const Complete = ({ route }) => {
    ...
    useEffect(() => {
        const fetchProducts = async() => {
            const q = query(collection(db, "Orders"), where("supplier", "==", route.params.id), where("status", "==", "Complete"));
            ...
            <View style = {styles.container}>
                ...
                <ScrollView style = {styles.scrollV}>
                    <View style = {styles.scroll}>
                        ...
                        <Text style = {styles.desc}> There are no orders completed yet.</Text>
                    </View>
                </ScrollView>
            </View>
        );
                ...
                <ImageBackground source = {require('../images/cover8.png')} resizeMethod = 'scale'
                    ...
                    {products.map((item, key) => (
                        <TouchableOpacity style = {styles.hold}
                            key = {key}
                            onPress = {() => {toggleModal(item.invoice)}}>
                    ...
                    <Modal
                        ...
                        <View style = {styles.container}>
                            <Item_S invoice = {invoice} supplier = {route.params.id} />
